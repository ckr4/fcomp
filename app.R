...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      numericInput("no_samples", "# of samples per treatment", 
                               value=5),
  ...
  output$ncp <- renderText({
    ...
    output$fco <- renderText({
      fco <- fcutoff(as.numeric(input$alpha), as.numeric(input$no_trtmnts), 
                     as.numeric(input$no_samples))
      op_fco <- paste("Critical F-Value:", fco)
      op_fco
    })
    
    # calculate and display non-centrality parameter
    ncp <- (sum(values$means^2 * input$no_samples) - 
      (sum(values$means * input$no_samples)^2) / 
      (input$no_trtmnts * input$no_samples)) / input$var
  ...
  output$f_plot <- 
    ...
    renderPlot({
      ...
      if (length(values$means)==input$no_trtmnts) {
        fplots(input$no_trtmnts, input$no_samples, values$means, input$var, 
